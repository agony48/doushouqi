...
public class FlyBanner extends RelativeLayout {
    ...
    private void switchToPoint(final int currentPoint) {
        ...
        if(!isAutoPlay){
            setOnSlide(isCount, currentPoint);
        }
     ...
     * 是否滚动
     */
    public void isAutoPlay(boolean isPlay) {
        isAutoPlay = isPlay;
    }
    //点击事件
    ...
    public void setOnItemSlideListener(OnItemSlideListener listener) {
        this.onItemSlideListener = listener;
    }

    public interface OnItemSlideListener {
        void onItemSlide(int count,int bSlide);
    }

    private void setOnSlide(int count,int bSlide){
        if(onItemSlideListener!=null){
            onItemSlideListener.onItemSlide(count,bSlide);
        }
    }
}
